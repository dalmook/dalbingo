...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function checkForLines() {
        ...
        gridArray.forEach(row => {
            if (row.every(cell => cell)) newLines++;
        });

        // Check columns
        for (let col = 0; col < boardSize; col++) {
            let column = gridArray.map(row => row[col]);
            if (column.every(cell => cell)) newLines++;
        }

        // Check diagonals
        let diag1 = gridArray.map((row, idx) => row[idx]);
        let diag2 = gridArray.map((row, idx) => row[boardSize - idx - 1]);
        if (diag1.every(cell => cell)) newLines++;
        if (diag2.every(cell => cell)) newLines++;

        // Update lines completed
        if (newLines > linesCompleted) {
            linesCompleted = newLines;
            // Optionally, draw lines on the grid
            // This can be implemented with additional CSS or canvas
        }
    }
